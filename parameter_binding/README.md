- [绑定矩阵（binding matrix）](#绑定矩阵binding-matrix)
- [示例代码](#示例代码)
...
|  | T(lvalue) | T(rvalue) | T& | T&& | const T(lvalue) | const T(rvalue) | const T& | const T&& |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T | 拷贝构造 | 移动构造 | 拷贝构造 | 移动构造 | 移动构造 | 拷贝构造 | 拷贝构造 | 拷贝构造 |
| T& | 引用绑定 | No | 引用绑定 | 引用绑定 | No | No | No | No |
| T&& | No | 引用绑定 | No | No(Note3) | No | No | No | No(Note3) |
| const T | 拷贝构造 | 移动构造 | 拷贝构造 | 移动构造 | 拷贝构造 | 移动构造 | 拷贝构造 | 移动构造 |
| const T& | 引用绑定 | 引用绑定 | 引用绑定 | 引用绑定 | 引用绑定 | 引用绑定 | 引用绑定 | 引用绑定 |
| const T&& | No | 引用绑定 | No | No(Note3) | No | 引用绑定 | No | No(Note3) |


说明：
1. 当传入右值构造一个对象时，如果没有定义移动构造函数，则会默认调用拷贝构造函数
